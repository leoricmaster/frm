...
- 日期：2026-09-01
- 状态：已与发起人逐节确认
- 依据：[统一代码管理体系设计](./2026-08-31-excavator-code-management-design.md) §5.3/§6.4/§6.5/§7.2、[原型验证结论](./2026-09-07-prototype-verification.md)（二阶段实测结果与边界见其 §1/§3）
...
二阶段为现有环境（gitlab + runner）加配 Harbor 与 MinIO，验证三个场景：
...
| Harbor 接入形态 | 组件容器直接写进主 `docker-compose.yml`（与 GitLab/runner 同网络同生命周期；实测改为独立 compose——harbor.yml 入库、生成 compose 不入库，见验证结论 §3-6） |
...
- Harbor 组件来自官方 offline installer 解包，逐个翻译为 compose services（实测以独立 compose 落地，见 §2 表）；容器名加 `frm-` 前缀，内部走 frm-net 主机名（harbor-core、harbor-registry 等），对外仅暴露 8084。
...
### 场景 1：镜像出口收敛（§5.3/§6.4）
- 验证点：`frm-ci` 直推回拉与 `docker-hub-proxy` 代理缓存；判定标准：build→push→删本地镜像→从 Harbor 地址 pull 成功，经 proxy 拉取后 Harbor 仓库页出现缓存记录（"出口收敛"截图证据）。

### 场景 2：大文件指针 + 现场回传 + 训练产物（§6.4/§7.2）
- 验证点：manifest 指针清单（大文件本体不进 git）取数、投放区 write-only 回传、训练产物入库；判定标准：CI 读 manifest 从 MinIO 拉取并过 sha256 校验流水线绿，投放区列目录被拒、上传成功（平台侧取走归档 `dataset-model`），"模型权重"推入 `training-output` 且 manifest.json 记 MinIO 路径。

### 场景 3：CI 凭据集成（§6.5）
- 验证点：HARBOR_USER/HARBOR_PASS、MINIO_KEY/MINIO_SECRET 配为 GitLab 受保护 CI 变量并新增 stage 引用；判定标准：job 日志中凭据打码。

实测结果见[验证结论](./2026-09-07-prototype-verification.md) §1 阶段二。
...
| 交付物 | 验收标准 |
|---|---|
| 更新后的 `docker-compose.yml`（+MinIO 2 服务）与 Harbor 独立 compose（harbor.yml 入库） | 两处 `docker compose up -d` 全部 healthy（共 ~15 服务） |
...
五步均为已执行完毕的时序（MinIO 先行、Harbor 镜像后台慢拉并行——该决策点已记于 §2 表；镜像到位后加 Harbor 服务、sudo 配 insecure-registries、建项目，场景 3 凭据集成贯穿流水线，结论页更新提交）。
实测时序与结果见[验证结论](./2026-09-07-prototype-verification.md) §1 阶段二。
